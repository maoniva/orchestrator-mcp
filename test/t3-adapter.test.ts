...
class FakeClient implements T3ClientLike {
  ...
  async getShellSnapshot(): Promise<T3ShellSnapshot> {
    return { ...snapshot, threads: this.activeThreads };
  }
...
describe("T3Adapter", () => {
  it("lists live reasoning levels and model options", async () => {
    const adapter = new T3Adapter(new FakeClient(), true);
  ...
  it("creates and starts a project-checkout thread with inherited model options", async () => {
    ...
    const adapter = new T3Adapter(client, true);
  ...
  it("adds T3's worktree bootstrap fields", async () => {
    const client = new FakeClient();
    const adapter = new T3Adapter(client, true);
    ...
    expect(result.branch).toBe("agent/payment-fix");
